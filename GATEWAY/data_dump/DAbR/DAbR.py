...
def CreateModel (data, no_of_attr, isReturn):
	...
	largest=0.0
	for ip, attr in vec_dict.iteritems():
		sum_sq = 0
		for i in range(no_of_attr):
	...
	if isReturn:
		return model
	with open("DAbR_model.pkl", 'wb') as f:
		pickle.dump(model, f)
...
def readModel():
	with open("DAbR_model.pkl", 'rb') as f:
		model = pickle.load(f)
	return model
...
def main():
	black_data, no_of_attr = create_data_dictionary(create_data_list(sys.argv[1]))		
	if isnewModel:
		CreateModel(black_data, no_of_attr, False)

	white_data, no_of_attr = create_data_dictionary(create_data_list(sys.argv[2]))	
